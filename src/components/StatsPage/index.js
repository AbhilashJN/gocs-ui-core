...
const StatsPage = ({player,setLoading,loading,getStatsForPlayerApi})=>{
	...
	useEffect(()=>{
        const parseDemo = async(player)=>{
            ...
            setStats({})
            console.log('fetch')
            const resp = await getStatsForPlayerApi(player,statType);
	...
	},[player, statType, setLoading, getStatsForPlayerApi])
