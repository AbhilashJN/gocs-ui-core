...
const HeatmapsPage=({player,setLoading, mapName,getDeathPositionsForPlayerApi})=>{
	...
	useEffect(()=>{
        const parseDemo = async(player)=>{
            ...
            clearPlot()
            console.log('fetch')
            const resp = await getDeathPositionsForPlayerApi(player)
	...
	},[getDeathPositionsForPlayerApi, player, setLoading])
